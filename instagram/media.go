...
func (s *MediaService) Search(opt *Parameters) ([]Media, *ResponsePagination, error) {
	...
	if opt != nil {
		...
		if opt.Count != 0 {
			//The count parameter is not "officially" supported in Instagram.
			// However we found that it works as of 6/2015. There is no guarantee that it will work always.
